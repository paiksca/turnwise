...
Added local audio transcription. `transcribe_session` merges a folder of per-participant recordings into one speaker-labeled transcript, which gives exact attribution without a diarization model.
...
Thresholds are fitted to the fixture set and remain provisional. No inter-rater reliability against trained human coders has been established. See `docs/METHODOLOGY.md`.

### Changing the rubric

Any edit to `src/lexicon.ts` or `THRESHOLDS` changes what the numbers mean. Bump `RUBRIC_VERSION`, run `npm run calibrate`, and note here what moved and why. A lexicon change that fixes one case and quietly breaks three others is the normal failure mode, which is what the calibration suite exists to catch.
